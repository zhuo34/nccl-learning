...
static ncclResult_t scheduleCollTasksToPlan(
  ...
  while (nPlanColls!=0 && !ncclIntruQueueEmpty(&planner->collTaskQueue)) {
    ...
    if (!plan->kernelSpecialized) {
      // ncclPrepareTasks() 里调用 ncclDevFuncId() 获得 devFuncId
      // ncclDevKernelForFunc 数组在 `src/device/generate.py` 生成的 `host_table.cc` 里定义
      // `src/device/generate.py` 生成对应 collative 的 cu 文件，其中有各函数的定义，通过包含 `src/device` 下的头文件定义
...
static ncclResult_t taskAppend(struct ncclComm* comm, struct ncclInfo* info) {
  ...
  if (info->coll == ncclFuncSend || info->coll == ncclFuncRecv) {
    ...
    // ncclGroupCommJoin 把当前 comm 链接到 ncclGroupCommHead 中
  ...
  if (info->stream != planner->streamRecent || planner->streams == nullptr) {
    ...
    // planner streams 是个链表，这里遍历把新 stream 链到队尾（cudaStream_t 类似 ncclComm_t，是指向 stream 结构的指针）
    while (true) {
